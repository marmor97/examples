...
## Plot
#heart
heart_plot1 <- ggplot(synchro, aes(min, HR1)) + geom_line()
heart_plot2 <- ggplot(synchro, aes(min, HR2)) + geom_line()

gridExtra::grid.arrange(heart_plot1, heart_plot2)

#resp
resp_plot1 <- ggplot(synchro, aes(min, Resp1)) + geom_line()
resp_plot2 <- ggplot(synchro, aes(min, Resp2)) + geom_line()

gridExtra::grid.arrange(resp_plot1, resp_plot2)
...
```{r}
#We are choosing to only include study 4
datty_4 <- datty[datty$study==4,]

#generating new columns for previous HR and resps and change in HR and resp
mommy <- datty_4 %>% group_by(group, condition, trial, study) %>% mutate(HR1 = HR1, HR2 = HR2, HR1_future = lead(HR1, 1), HR2_future = lead(HR2, 1), HR1_change = (HR1_future-HR1), HR2_change = (HR2_future-HR2), Resp1_future = lead(Resp1, n= 1), Resp2_future = lead(Resp2, 1), Resp1_change = (Resp1_future-Resp1), Resp2_change = (Resp2_future-Resp2))
...
save(m3, file = "model.rda")
summary(m3)

#second model but simple
m4 <- lmer(HR_change_self ~ 0 + condition (HR_self + HR_other):condition + 
...
groups <- as.numeric(as.character(unique(mommy$group[mommy$study==4]))) # List all pairs
...
w2 <- as.data.frame(matrix(0, nrow = 0, ncol = 12)) #making dataframe to store surrogate data

for (i in 1:nrow(surrogate_list)){  # loop through all combinations
    x <- subset(mommy, group==surrogate_list$a[i]) # subset data from the first pair
    y <- subset(mommy, group==surrogate_list$b[i]) # subset data from the second pair   
    group <- c(800 + ((1:4)*i))                            # create new pair id
    for (co in c("S y","T u", "M o", "C o")){ # loop through conditions
       if (co %in% unique(x$condition) & co %in% unique(y$condition)){  # check both pairs have data 4 that condit.
           ...
           if (nrow(z2) > nrow(z1)) { 
            ...
            w1 <- z1 %>% mutate(	# assemble new pair combining the 2 pairs
               HR2 = z2$HR2,
               Resp2 = z2$Resp2,
               HR2_lead = z2$HR2_future,
               Resp2_lead = z2$Resp2_future,
               HR2_change = z2$HR2_change, 
               Resp2_change = z2$Resp2_change)
            w1$pair_number <- i
               
            w2 <- rbind(as.data.frame(w2), as.data.frame(w1))
       } }}

#making surrogate data long format
d_hr_change <- 
  gather(w2, # data
         participant, HR_change_self, #new vars
         HR1_change, HR2_change) %>% #old vars
...
d_resp_future <- 
  gather(w2, # data
         ...
         Resp1_future, Resp1_future) %>% #old vars
  select(#drop irrelevant vars
    time, resp_future, filename,
    study, group, condition, trial, participant) %>% 
  mutate(#create unique participant ID
    participant = parse_number(as.character(group))*10 + parse_number(participant))
colnames(w2)
...
#collecting in one dataframe
grandma <- d_hr_self
grandma$hr_change <- d_hr_change$HR_change_self
grandma$resp_change <- d_resp_change$Resp_change_self
grandma$resp_self <- d_resp_self$Resp_self
grandma$hr_future <- d_hr_future$hr_future
grandma$resp_future <- d_resp_future$resp_future


#doing the same as above but reversed for 1 and 2 so we can use it for columns with other person data 
cousin <- w2 %>% group_by(group, condition, trial, study) %>% mutate(HR2_future = lead(HR1, n= 1), HR1_future = lead(HR2, 1), HR2_change = (HR1_future-HR1), HR1_change = (HR2_future-HR2), Resp2_future = lead(Resp1, n= 1), Resp1_future = lead(Resp2, 1), Resp2_change = (Resp1_future-Resp1), Resp1_change = (Resp2_future-Resp2), HR1_fake = HR2, HR2_fake = HR1, Resp1_fake = Resp2, Resp2_fake = Resp1)
...
grandma$HR_self_other <- d_hr_other$HR_other
grandma$Resp_other <- d_resp_other$Resp_other
grandma$hr_change_other <- d_hr_change_other$HR_change_other
grandma$resp_change_other <- d_resp_change_other$Resp_change_other
colnames(grandma) <- c("time", "HR_self", "filename", "study", "group", "condition", "trial", "participant", "HR_change", "Resp_change", "Resp_self", "HR_future", "Resp_future", "HR_other", "Resp_other", "HR_change_other", "Resp_change_other")

#write.csv(grandma, "surrogate_grandma.csv", row.names = F)
...
grandma$surrogate <- c(1)
stepgrandparents <-  grandparents
stepgrandparents$surrogate <- c(0) 

#merging
greatgrand <- merge(stepgrandparents, grandma, all = T)
...
#save(m3, sur3, file = "supermodels.rda")
```
